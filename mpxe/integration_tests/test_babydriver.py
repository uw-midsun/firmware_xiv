...
class TestBabyDriver(int_test.IntTest):
    ...
    def test_gpio_set(self):
        time.sleep(1)
        self.babydriver.sim.get_gpio(proj=self.babydriver, port='A', pin=5) # To get intial states
        
        time.sleep(1)
        gpio_set.gpio_set('A', 5, True)
        
        time.sleep(1)
        self.babydriver.sim.get_gpio(proj=self.babydriver, port='A', pin=5) # To get states after the set
