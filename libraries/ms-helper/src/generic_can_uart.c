...
// CanUartRxCb
static void prv_generic_can_uart_rx_handler(const CanUart *can_uart,
...
static StatusCode prv_tx(const GenericCan *can, const GenericCanMsg *msg) {
  ...
  if (gcu->base.interface != &s_interface || gcu->can_uart != &s_can_uart) {
    return status_msg(STATUS_CODE_INVALID_ARGS,
                      "GenericCan not aligned to GenericCanUart.");
  }
  return can_uart_req_slave_tx(gcu->can_uart, msg->id, msg->extended,
                               &msg->data, msg->dlc);
}
...
StatusCode generic_can_uart_init(GenericCanUart *can_uart, UartPort port) {
  ...
  memset(can_uart->base.rx_storage, 0,
