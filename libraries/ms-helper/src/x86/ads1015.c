...
static bool prv_channel_is_enabled(Ads1015Storage *storage,
...
static void prv_timer_callback(SoftTimerId id, void *context) {
  ...
  if (prv_channel_is_enabled(storage, current_channel)) {
    storage->channel_readings[current_channel] =
        ADS1015_CHANNEL_ARBITRARY_READING;
  ...
  current_channel =
      (Ads1015Channel)(__builtin_ffs(storage->pending_channel_bitset) - 1);
  // Update so that the ADS1015 reads from the next channel.
  prv_set_channel(storage, current_channel);
  soft_timer_start(ADS1015_CHANNEL_UPDATE_PERIOD_US, prv_timer_callback,
                   storage, NULL);
}
...
StatusCode ads1015_init(Ads1015Storage *storage, I2CPort i2c_port,
  ...
  return soft_timer_start(ADS1015_CHANNEL_UPDATE_PERIOD_US, prv_timer_callback,
                          storage, NULL);
}
