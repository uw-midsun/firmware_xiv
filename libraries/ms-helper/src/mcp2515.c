...
static const Mcp2515TxBuffer s_tx_buffers[] = {
    {.id = MCP2515_LOAD_TXB0SIDH,
     .data = MCP2515_LOAD_TXB0D0,
     .rts = MCP2515_RTS_TXB0},
    {.id = MCP2515_LOAD_TXB1SIDH,
     .data = MCP2515_LOAD_TXB1D0,
     .rts = MCP2515_RTS_TXB1},
    {.id = MCP2515_LOAD_TXB2SIDH,
     .data = MCP2515_LOAD_TXB2D0,
     .rts = MCP2515_RTS_TXB2},
};

static const Mcp2515RxBuffer s_rx_buffers[] = {
    {.id = MCP2515_READ_RXB0SIDH,
     .data = MCP2515_READ_RXB0D0,
     .int_flag = MCP2515_CANINT_RX0IE},
    {.id = MCP2515_READ_RXB1SIDH,
     .data = MCP2515_READ_RXB1D0,
     .int_flag = MCP2515_CANINT_RX1IE},
};
...
static void prv_read(Mcp2515Storage *storage, uint8_t addr, uint8_t *read_data,
                     size_t read_len) {
  uint8_t payload[] = {MCP2515_CMD_READ, addr};
  spi_exchange(storage->spi_port, payload, sizeof(payload), read_data,
               read_len);
}

static void prv_write(Mcp2515Storage *storage, uint8_t addr,
  ...
  spi_exchange(storage->spi_port, payload, sizeof(payload), write_data,
               write_len);
}

// See 12.10: *addr = (data & mask) | (*addr & ~mask)
static void prv_bit_modify(Mcp2515Storage *storage, uint8_t addr, uint8_t mask,
                           uint8_t data) {
...
static void prv_handle_rx(Mcp2515Storage *storage, uint8_t int_flags) {
  for (size_t i = 0; i < SIZEOF_ARRAY(s_rx_buffers); i++) {
    ...
    if (int_flags & rx_buf->int_flag) {
      ...
      spi_exchange(storage->spi_port, id_payload, sizeof(id_payload),
                   (uint8_t *)&read_id_regs, sizeof(read_id_regs));
      Mcp2515Id id = {
      ...
      spi_exchange(storage->spi_port, data_payload, sizeof(data_payload),
...
StatusCode mcp2515_init(Mcp2515Storage *storage,
  ...
  // Leave config mode
  uint8_t opmode = (settings->loopback ? MCP2515_CANCTRL_OPMODE_LOOPBACK
                                       : MCP2515_CANCTRL_OPMODE_NORMAL);
  prv_bit_modify(storage, MCP2515_CTRL_REG_CANCTRL, MCP2515_CANCTRL_OPMODE_MASK,
                 opmode);
  ...
  return gpio_it_register_interrupt(&settings->int_pin, &it_settings,
                                    INTERRUPT_EDGE_FALLING, prv_handle_int,
                                    storage);
}
...
StatusCode mcp2515_tx(Mcp2515Storage *storage, uint32_t id, bool extended,
  ...
  spi_exchange(storage->spi_port, (uint8_t *)&data_payload,
               sizeof(data_payload), NULL, 0);
