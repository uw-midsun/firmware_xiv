...
static void prv_tx_handler(void *context) { (void)context; }
...
static void prv_rx_handler(void *context) {
  ...
  while (
      can_hw_receive(&rx_msg.id, &rx_msg.extended, &rx_msg.data, &rx_msg.dlc)) {
    for (size_t i = 0; i < NUM_GENERIC_CAN_RX_HANDLERS; i++) {
      if (gch->base.rx_storage[i].rx_handler != NULL &&
          (rx_msg.id & gch->base.rx_storage[i].mask) ==
              gch->base.rx_storage[i].filter) {
        gch->base.rx_storage[i].rx_handler(&rx_msg,
                                           gch->base.rx_storage[i].context);
        break;
      }
    }
  }
...
static void prv_bus_error_handler(void *context) {
  ...
  soft_timer_start_millis(CAN_BUS_OFF_RECOVERY_TIME_MS,
                          prv_bus_error_timeout_handler, gch, NULL);
}
...
static StatusCode prv_tx(const GenericCan *can, const GenericCanMsg *msg) {
  ...
  if (gch->base.interface != &s_interface) {
    return status_msg(STATUS_CODE_INVALID_ARGS,
                      "GenericCan not aligned to GenericCanHw.");
  } else if (can == NULL || msg == NULL) {
    return status_code(STATUS_CODE_INVALID_ARGS);
  }

  return can_hw_transmit(msg->id, msg->extended, (uint8_t *)&msg->data,
                         msg->dlc);
}

// register_rx
static StatusCode prv_register_rx(GenericCan *can, GenericCanRx rx_handler,
...
StatusCode generic_can_hw_init(GenericCanHw *can_hw,
                               const CanHwSettings *settings,
                               EventId fault_event) {
