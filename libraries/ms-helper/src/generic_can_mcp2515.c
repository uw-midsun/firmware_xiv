...
static void prv_rx_handler(uint32_t id, bool extended, uint64_t data,
  ...
  for (size_t i = 0; i < NUM_GENERIC_CAN_RX_HANDLERS; i++) {
    if (gcmcp->base.rx_storage[i].rx_handler != NULL &&
        (id & gcmcp->base.rx_storage[i].mask) ==
            gcmcp->base.rx_storage[i].filter) {
      ...
      gcmcp->base.rx_storage[i].rx_handler(&msg,
                                           gcmcp->base.rx_storage[i].context);
      break;
    }
  }
}
...
static StatusCode prv_tx(const GenericCan *can, const GenericCanMsg *msg) {
  ...
  if (gcmcp->base.interface != &s_interface || gcmcp->mcp2515 != &s_mcp2515) {
    return status_msg(STATUS_CODE_INVALID_ARGS,
                      "GenericCan not aligned to GenericCanMcp2515.");
  }
  return mcp2515_tx(gcmcp->mcp2515, msg->id, msg->extended, msg->data,
                    msg->dlc);
}
...
StatusCode generic_can_mcp2515_init(GenericCanMcp2515 *can_mcp2515,
  ...
  status_ok_or_return(
      mcp2515_register_rx_cb(&s_mcp2515, prv_rx_handler, can_mcp2515));
  can_mcp2515->mcp2515 = &s_mcp2515;

  memset(can_mcp2515->base.rx_storage, 0,
         sizeof(GenericCanRx) * NUM_GENERIC_CAN_RX_HANDLERS);
