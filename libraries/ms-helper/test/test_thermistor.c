...
StatusCode TEST_MOCK(adc_read_converted)(AdcChannel adc_channel,
                                         uint16_t *reading) {
  switch (adc_channel) {
  case (ADC_CHANNEL_0):
...
StatusCode TEST_MOCK(adc_get_channel)(GpioAddress address,
...
void test_thermistor_normal(void) {
  ...
  TEST_ASSERT_UINT16_WITHIN(THERMISTOR_TEMPERATURE_TOLERANCE_DECICELSIUS, 100,
                            reading);
}
...
void test_thermistor_min_temp(void) {
  ...
  TEST_ASSERT_OK(thermistor_get_temp(&storage, &reading));
  LOG_DEBUG("Temperature: %" PRIuLEAST16 "\n", reading);
  TEST_ASSERT_UINT16_WITHIN(THERMISTOR_TEMPERATURE_TOLERANCE_DECICELSIUS, 0,
                            reading);
}
...
void test_thermistor_max_temp(void) {
  ...
  TEST_ASSERT_OK(thermistor_get_temp(&storage, &reading));
  LOG_DEBUG("Temperature: %" PRIuLEAST16 "\n", reading);
  TEST_ASSERT_UINT16_WITHIN(THERMISTOR_TEMPERATURE_TOLERANCE_DECICELSIUS, 1000,
                            reading);
}
