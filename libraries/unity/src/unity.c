...
void UnityAssertEqualIntArray(UNITY_INTERNAL_PTR expected, UNITY_INTERNAL_PTR actual,
  ...
  /* If style is UNITY_DISPLAY_STYLE_INT, we'll fall into the default case rather than the INT16 or
   * INT32 (etc) case as UNITY_DISPLAY_STYLE_INT includes a flag for UNITY_DISPLAY_RANGE_AUTO, which
   * the width-specific variants do not. Therefore remove this flag. */
  switch (style & (UNITY_DISPLAY_STYLE_T)(~UNITY_DISPLAY_RANGE_AUTO)) {
    case UNITY_DISPLAY_STYLE_HEX8:
    case UNITY_DISPLAY_STYLE_INT8:
    case UNITY_DISPLAY_STYLE_UINT8:
      while (elements--) {
        if (*(UNITY_PTR_ATTRIBUTE const _US8 *)ptr_exp !=
            *(UNITY_PTR_ATTRIBUTE const _US8 *)ptr_act) {
          UnityTestResultsFailBegin(lineNumber);
    ...
    case UNITY_DISPLAY_STYLE_UINT16:
      while (elements--) {
        if (*(UNITY_PTR_ATTRIBUTE const _US16 *)ptr_exp !=
            *(UNITY_PTR_ATTRIBUTE const _US16 *)ptr_act) {
          UnityTestResultsFailBegin(lineNumber);
          UnityPrint(UnityStrElement);
          UnityPrintNumberUnsigned(num_elements - elements - 1);
          UnityPrint(UnityStrExpected);
    ...
    case UNITY_DISPLAY_STYLE_UINT64:
      while (elements--) {
        if (*(UNITY_PTR_ATTRIBUTE const _US64 *)ptr_exp !=
            *(UNITY_PTR_ATTRIBUTE const _US64 *)ptr_act) {
          UnityTestResultsFailBegin(lineNumber);
          UnityPrint(UnityStrElement);
          UnityPrintNumberUnsigned(num_elements - elements - 1);
          UnityPrint(UnityStrExpected);
    ...
    default:
      while (elements--) {
        if (*(UNITY_PTR_ATTRIBUTE const _US32 *)ptr_exp !=
            *(UNITY_PTR_ATTRIBUTE const _US32 *)ptr_act) {
          UnityTestResultsFailBegin(lineNumber);
          UnityPrint(UnityStrElement);
          UnityPrintNumberUnsigned(num_elements - elements - 1);
          UnityPrint(UnityStrExpected);
