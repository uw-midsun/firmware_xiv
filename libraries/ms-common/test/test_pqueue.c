...
void setup_test(void) { pqueue_init(&gv_queue, gv_nodes, TEST_PQUEUE_SIZE); }
...
void test_pqueue_desc_order(void) {
  ...
  for (size_t i = 0; i < SIZEOF_ARRAY(prios); i++) {
    ...
    TEST_ASSERT_MESSAGE(last_prio <= *prio,
                        "Last priority was lower than new priority!");
    last_prio = *prio;
  }
}
...
void test_pqueue_out_of_space(void) {
  ...
  // NULL.
  for (size_t i = 0; i < TEST_PQUEUE_SIZE + 5; i++) {
    TEST_ASSERT_EQUAL(((i + 1) < TEST_PQUEUE_SIZE) ? (void *)i : NULL,
                      pqueue_pop(&gv_queue));
  }
}
