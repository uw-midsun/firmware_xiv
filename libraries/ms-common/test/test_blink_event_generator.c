...
void test_blink_event_generator_valid(void) {
  ...
  TEST_ASSERT_OK(blink_event_generator_start(&storage));
  ...
  blink_event_generator_start(&storage);
  TEST_ASSERT_NOT_OK(event_process(&e));  // don't raise immediately
  delay_us(interval_us);
  TEST_ASSERT_OK(event_process(&e));
  TEST_ASSERT_EQUAL(TEST_EVENT_ID, e.id);
  TEST_ASSERT_EQUAL(0, e.data);  // still toggling
}
...
void test_blink_event_generator_init_default_first_value_is_0(void) {
  const uint32_t interval_us = 5000;
  BlinkEventGeneratorSettings settings = {
    .interval_us = interval_us, .event_id = TEST_EVENT_ID,
...
void test_multiple_blink_event_generators(void) {
  ...
  const uint32_t interval_us1 = 4000;
  ...
  const uint32_t interval_us2 = 6000;  // must be greater than interval_us1
  ...
  TEST_ASSERT_OK(blink_event_generator_start(&storage1));
  TEST_ASSERT_OK(blink_event_generator_start(&storage2));
  TEST_ASSERT_NOT_OK(event_process(&e));
  ...
  // cancel the second one, restart the first one, make sure everything's valid
  blink_event_generator_stop(&storage2);
  blink_event_generator_start(&storage1);
