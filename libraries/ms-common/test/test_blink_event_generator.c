...
//   restart TEST_ASSERT_OK(event_process(&e)); TEST_ASSERT_EQUAL(TEST_EVENT_ID_2, e.id);
//   TEST_ASSERT_EQUAL(0, e.data);  // resets to first_value
//   delay_us(interval_us);
//   TEST_ASSERT_OK(event_process(&e));
...
//   valid blink_event_generator_stop(&storage2); TEST_ASSERT_OK(event_process(&e));  // eat the
