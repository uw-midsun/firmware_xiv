...
StatusCode prv_bts7xxx_enable_pin(Bts7xxxEnablePin *pin) {
...
void prv_bts7xxx_fault_handler_enable_cb(SoftTimerId timer_id, void *context) {
  Bts7xxxEnablePin *pin = context;
  prv_bts7xxx_fault_handler_cb(timer_id, context);
  prv_bts7xxx_enable_pin(pin);
}
...
StatusCode prv_bts7xxx_handle_fault_pin(Bts7xxxEnablePin *pin) {
  if (!pin->fault_in_progress) {
    ...
    if (prv_bts7xxx_get_pin_enabled(pin)) {
      status_ok_or_return(prv_bts7xxx_disable_pin(pin));
      soft_timer_start(BTS7XXX_FAULT_RESTART_DELAY_US, prv_bts7xxx_fault_handler_enable_cb, pin,
                       &pin->fault_timer_id);
    } else {
      soft_timer_start(BTS7XXX_FAULT_RESTART_DELAY_US, prv_bts7xxx_fault_handler_cb, pin,
                       &pin->fault_timer_id);
    }
  }
