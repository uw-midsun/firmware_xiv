...
StatusCode adt7476a_set_speed(I2CPort port, uint8_t speed_percent, AdtPwmPort pwm_port,
  ...
  if (pwm_port == ADT_PWM_PORT_1) {
    status_ok_or_return(i2c_write(port, ADT7476A_PWM_1, &real_speed, SET_SPEED_NUM_BYTES));

  } else if (pwm_port == ADT_PWM_PORT_2) {
    status_ok_or_return(i2c_write(port, ADT7476A_PWM_3, &real_speed, SET_SPEED_NUM_BYTES));
...
StatusCode adt7476a_get_status(I2CPort port, uint8_t adt7476a_i2c_read_address,
  ...
  status_ok_or_return(
      i2c_read(port, ADT7476A_INTERRUPT_STATUS_REGISTER_1, register_1_data, ADT7476A_REG_SIZE));
  status_ok_or_return(
      i2c_read(port, ADT7476A_INTERRUPT_STATUS_REGISTER_2, register_2_data, ADT7476A_REG_SIZE));
...
StatusCode adt7476a_init(Adt7476aStorage *storage, Adt7476aSettings *settings) {
  ...
  uint8_t strt_config_data = ADT7476A_CONFIG_REG_1_MASK;
  uint8_t smbalert_config_data = ADT7476A_CONFIG_REG_3_MASK;

  // set STRT bit to on
  status_ok_or_return(
      i2c_write(settings->i2c, ADT7476A_CONFIG_REGISTER_1, &strt_config_data, ADT7476A_REG_SIZE));

  // configure pwm to manual mode
  status_ok_or_return(
      i2c_write(settings->i2c, ADT7476A_FAN_MODE_REGISTER_1, &fan_config_data, ADT7476A_REG_SIZE));
  status_ok_or_return(
      i2c_write(settings->i2c, ADT7476A_FAN_MODE_REGISTER_3, &fan_config_data, ADT7476A_REG_SIZE));

  // set pin 10 to SMBALERT rather than pwm output
  status_ok_or_return(i2c_write(settings->i2c, ADT7476A_CONFIG_REGISTER_3, &smbalert_config_data,
                                ADT7476A_REG_SIZE));
