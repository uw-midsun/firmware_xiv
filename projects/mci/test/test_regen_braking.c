...
void setup_test(void) {
  ...
  TEST_ASSERT_EQUAL(true, get_regen_braking_state());
}
...
void test_regen_state_off(void) {
  ...
  CAN_TRANSMIT_REGEN_BRAKING(&req, false);
  MS_TEST_HELPER_CAN_TX_RX(MCI_CAN_EVENT_TX, MCI_CAN_EVENT_RX);
  TEST_ASSERT_EQUAL(false, get_regen_braking_state());
}
...
void test_regen_state_on(void) {
  ...
  CAN_TRANSMIT_REGEN_BRAKING(&req, true);
  MS_TEST_HELPER_CAN_TX_RX(MCI_CAN_EVENT_TX, MCI_CAN_EVENT_RX);
  TEST_ASSERT_EQUAL(true, get_regen_braking_state());
}
