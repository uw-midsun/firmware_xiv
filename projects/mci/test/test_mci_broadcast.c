...
static MotorCanFrameId s_frame_id_map[] = {
  [LEFT_MOTOR_CONTROLLER*NUM_TEST_MCI_MESSAGES + TEST_MCI_VELOCITY_MESSAGE] = MOTOR_CAN_LEFT_VELOCITY_MEASUREMENT_FRAME_ID,
  [RIGHT_MOTOR_CONTROLLER*NUM_TEST_MCI_MESSAGES + TEST_MCI_VELOCITY_MESSAGE] = MOTOR_CAN_RIGHT_VELOCITY_MEASUREMENT_FRAME_ID,
  [LEFT_MOTOR_CONTROLLER*NUM_TEST_MCI_MESSAGES + TEST_MCI_BUS_MEASUREMENT_MESSAGE] = MOTOR_CAN_LEFT_BUS_MEASUREMENT_FRAME_ID,
  [RIGHT_MOTOR_CONTROLLER*NUM_TEST_MCI_MESSAGES + TEST_MCI_BUS_MEASUREMENT_MESSAGE] = MOTOR_CAN_RIGHT_BUS_MEASUREMENT_FRAME_ID,
};

static StatusCode prv_handle_velocity(const CanMessage *msg, void *context, CanAckStatus *ack_reply) {
  ...
  TEST_ASSERT_EQUAL((uint16_t)(s_test_measurements.vehicle_velocity[LEFT_MOTOR_CONTROLLER] * 100), left_velocity);
  TEST_ASSERT_EQUAL((uint16_t)(s_test_measurements.vehicle_velocity[RIGHT_MOTOR_CONTROLLER] * 100), right_velocity);
  return STATUS_CODE_OK;
}

static StatusCode prv_handle_bus_measurement(const CanMessage *msg, void *context, CanAckStatus *ack_reply) {
  ...
  TEST_ASSERT_EQUAL((uint16_t)s_test_measurements.bus_measurements[LEFT_MOTOR_CONTROLLER].bus_voltage_v, left_voltage);
  TEST_ASSERT_EQUAL((uint16_t)s_test_measurements.bus_measurements[LEFT_MOTOR_CONTROLLER].bus_current_a, left_current);
  TEST_ASSERT_EQUAL((uint16_t)s_test_measurements.bus_measurements[RIGHT_MOTOR_CONTROLLER].bus_voltage_v, right_voltage);
  TEST_ASSERT_EQUAL((uint16_t)s_test_measurements.bus_measurements[RIGHT_MOTOR_CONTROLLER].bus_current_a, right_current);
  return STATUS_CODE_OK;
}

static void prv_send_measurements(MotorController controller, TestMciMessage message_type, bool reset) {
  ...
  if (message_type == TEST_MCI_VELOCITY_MESSAGE) {
    ...
    can_data.velocity_measurement.vehicle_velocity_ms = (!reset) * s_test_measurements.vehicle_velocity[controller];
  }
  else if (message_type == TEST_MCI_BUS_MEASUREMENT_MESSAGE) {
    can_data.bus_measurement.bus_voltage_v = (!reset) * s_test_measurements.bus_measurements[controller].bus_voltage_v;
    can_data.bus_measurement.bus_current_a = (!reset) * s_test_measurements.bus_measurements[controller].bus_current_a;
  }
...
void setup_test(void) {
  ...
  TEST_ASSERT_OK(can_register_rx_handler(SYSTEM_CAN_MESSAGE_MOTOR_CONTROLLER_VC, prv_handle_bus_measurement, NULL));
  TEST_ASSERT_OK(can_register_rx_handler(SYSTEM_CAN_MESSAGE_MOTOR_VELOCITY, prv_handle_velocity, NULL));
}
