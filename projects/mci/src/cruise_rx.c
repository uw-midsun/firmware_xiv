...
static StatusCode prv_cruise_command_rx(const CanMessage *msg, void *context, CanAckStatus *ack_reply) {
  (void)context;
  ...
  switch (cruise_command) {
    case EE_CRUISE_CONTROL_COMMAND_TOGGLE:
      if (s_target_velocity_ms >= MCI_MIN_CRUISE_VELOCITY_MS && s_target_velocity_ms <= MCI_MAX_CRUISE_VELOCITY_MS)
        drive_fsm_toggle_cruise();
      return STATUS_CODE_OK;
    case EE_CRUISE_CONTROL_COMMAND_INCREASE:
      if (drive_fsm_is_cruise())
        s_target_velocity_ms = MIN(MCI_MAX_CRUISE_VELOCITY_MS, s_target_velocity_ms+MCI_CRUISE_CHANGE_AMOUNT_MS);
      return STATUS_CODE_OK;
    case EE_CRUISE_CONTROL_COMMAND_DECREASE:
      if (drive_fsm_is_cruise())
        s_target_velocity_ms = MAX(MCI_MIN_CRUISE_VELOCITY_MS, s_target_velocity_ms-MCI_CRUISE_CHANGE_AMOUNT_MS);
      return STATUS_CODE_OK;
    default:
