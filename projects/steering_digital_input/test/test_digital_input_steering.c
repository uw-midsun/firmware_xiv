...
steering_digital_input_init(&s_steering_lookup_table);
 ...
 void test_steering_digital_input_can_high_beam_forward(SteeringDigitalInputConfiguration *s_steering_lookup_table) {

//Triggers interrupt for the high beam in the front 
GpioAddress pin_high_beam_forward
=s_steering_lookup_table[STEERING_DIGITAL_INPUT_HIGH_BEAM_FORWARD].address;
   TEST_ASSERT_OK(gpip_it_trigger_interrupt(&pin_high_beam_forward));


  Event e = { 0 };
  if (status_ok(event_process(&e))) {
TEST_ASSERT_EQUAL(e.id==STEERING_DIGITAL_INPUT_EVENT_HIGH_BEAM_FORWARD, 
event_process(&e));can_process_event(&e);
    }   
