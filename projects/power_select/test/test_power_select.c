...
// Take measurements + broadcast, then stop measuring
...
static StatusCode prv_power_select_ab_mv_cb(const CanMessage *msg, void *context,
  ...
  CAN_UNPACK_AUX_BATTERY_STATUS_MAIN_POWER_VOLTAGE(
      msg, &s_ab_mv_measurements[0], &s_ab_mv_measurements[1], &s_ab_mv_measurements[2],
      &s_ab_mv_measurements[3]);
  return STATUS_CODE_OK;
}
...
static StatusCode prv_power_select_db_mc_cb(const CanMessage *msg, void *context,
                                            CanAckStatus *ack_reply) {
  CAN_UNPACK_DCDC_BATTERY_STATUS_MAIN_POWER_CURRENT(
      msg, &s_db_mc_measurements[0], &s_db_mc_measurements[1], &s_db_mc_measurements[2],
      &s_db_mc_measurements[3]);
  return STATUS_CODE_OK;
}
...
void test_power_select_broadcast_works(void) {
  ...
  can_register_rx_handler(SYSTEM_CAN_MESSAGE_AUX_BATTERY_STATUS_MAIN_POWER_VOLTAGE,
                          prv_power_select_ab_mv_cb, NULL);
  can_register_rx_handler(SYSTEM_CAN_MESSAGE_DCDC_BATTERY_STATUS_MAIN_POWER_VOLTAGE,
                          prv_power_select_db_mc_cb, NULL);
