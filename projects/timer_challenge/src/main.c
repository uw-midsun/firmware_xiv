...
#define A_INC_PERIOD_MS 500  // milliseconds between a-increment
...
void prv_timer_callback(SoftTimerId timer_id, void *context) {
  ...
  // increment b every other time a is incremented
  ...
  soft_timer_start_millis(A_INC_PERIOD_MS, prv_timer_callback, storage, NULL);
}
...
int main() {
  ...
  soft_timer_start_millis(A_INC_PERIOD_MS,     // timer duration
