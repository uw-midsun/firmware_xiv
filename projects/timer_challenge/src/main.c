...
#define COUNTER_PERIOD_MS 500  // value passed into
...
void prv_timer_callback(SoftTimerId timer_id, void *context) {
  CounterStorage *storage = context;  // cast void* to our struct so we can use it

  // increment and print counter a
  storage->counter_a++;
  LOG_DEBUG("Counter A: %i\n", storage->counter_a);

  if (storage->counter_a % 2 == 0) {
    // increment and print counter b
    storage->counter_b++;
...
int main() {
  ...
  CounterStorage storage = { 0 };  // we use this to initialize a struct to be all 0
