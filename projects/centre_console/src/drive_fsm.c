...
FSM_STATE_TRANSITION(state_set_relays) {
  FSM_ADD_GUARDED_TRANSITION(DRIVE_FSM_INPUT_EVENT_MCI_RELAYS_CLOSED_DESTINATION_DRIVE, prv_destination_guard, state_drive); // destination: drive
  FSM_ADD_GUARDED_TRANSITION(DRIVE_FSM_INPUT_EVENT_MCI_RELAYS_CLOSED_DESTINATION_REVERSE, prv_destination_guard, state_reverse); // destination: reverse
  ...
  FSM_ADD_GUARDED_TRANSITION(DRIVE_FSM_INPUT_EVENT_MCI_RELAYS_OPENED_DESTINATION_PARKING, prv_destination_guard, state_set_ebrake); // destination: parking
...
static void prv_fault_output(Fsm *fsm, const Event *e, void *context) {
  ...
  relay_tx_relay_state_and_raise_event(&tx_req);
}
...
static void prv_set_ebrake_output(Fsm *fsm, const Event *e, void *context) {
  ...
  ebrake_tx_state_and_raise_event(&tx_req);
}
