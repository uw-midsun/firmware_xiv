...
static StatusCode prv_receive_velocity(const CanMessage *msg, 
                                       void *context,
                                       CanAckStatus *ack_reply) {
  uint16_t u_velocity_left, u_velocity_right = 0;
  CAN_UNPACK_MOTOR_VELOCITY(msg, &u_velocity_left, 
                                  &u_velocity_right);
  float velocity_left = (float) u_velocity_left,
        velocity_right = (float) u_velocity_right;
...
StatusCode speed_monitor_init(WatchdogTimeout timeout) {
  s_storage = (SpeedMonitorStorage) {
    .timeout = timeout
  };
  status_ok_or_return(
      can_register_rx_handler(SYSTEM_CAN_MESSAGE_MOTOR_VELOCITY, 
      prv_receive_velocity, NULL));
  watchdog_start(&s_storage.watchdog_storage, timeout, 
    prv_did_not_receive_speed_message, NULL);
  return STATUS_CODE_OK;
}
