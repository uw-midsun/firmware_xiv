...
typedef enum {
  ...
  NUM_DRIVE_FSM_INPUT_EVENTS // 45
} DriveFsmInputEvent;

typedef enum {
  DRIVE_FSM_OUTPUT_EVENT_DRIVE = NUM_DRIVE_FSM_INPUT_EVENTS, // 45
  DRIVE_FSM_OUTPUT_EVENT_REVERSE,
  DRIVE_FSM_OUTPUT_EVENT_FAULT,
  DRIVE_FSM_OUTPUT_EVENT_PARKING,
  DRIVE_FSM_OUTPUT_EVENT_NEUTRAL,
  NUM_DRIVE_FSM_OUTPUT_EVENTS // 50
} DriveFsmOutputEvent;

typedef enum {
  PEDAL_MONITOR_RX_TIMED_OUT = NUM_DRIVE_FSM_OUTPUT_EVENTS, // 50
  PEDAL_MONITOR_STATE_CHANGE,
  NUM_PEDAL_MONITOR_EVENTS // 52
} PedalMonitorEvent;

typedef enum {
  BATTERY_HEARTBEAT_EVENT_HEALTH_CHECK_REQUEST = NUM_PEDAL_MONITOR_EVENTS, // 52
  NUM_BATTERY_HEARTBEAT_EVENTS // 53
} BatteryHeartbeatEvent;

typedef enum {
  CENTRE_CONSOLE_BUTTON_PRESS_EVENT_DRIVE = NUM_BATTERY_HEARTBEAT_EVENTS, // 53
  ...
  NUM_CENTRE_CONSOLE_BUTTON_PRESS_EVENTS // 59
} CentreConsoleButtonPressEvent;

typedef enum {
  HAZARD_EVENT_ON = NUM_CENTRE_CONSOLE_BUTTON_PRESS_EVENTS, // 59
  HAZARD_EVENT_OFF,
  NUM_HAZARD_EVENTS // 61
} HazardEvent;

typedef enum {
  RACE_SWITCH_EVENT_OFF = NUM_HAZARD_EVENTS, // 61
  RACE_SWITCH_EVENT_ON,
  NUM_RACE_SWITCH_EVENTS // 63
} RaceSwitchEvent;

#define NUM_CENTRE_CONSOLE_EVENTS NUM_RACE_SWITCH_EVENTS // 63
