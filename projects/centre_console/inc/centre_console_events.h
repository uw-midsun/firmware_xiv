...
typedef enum {
  ...
  NUM_CENTRE_CONSOLE_POWER_EVENTS  // 8
} CentreConsolePowerEvent;

typedef enum {
  POWER_AUX_SEQUENCE_EVENT_BEGIN = NUM_CENTRE_CONSOLE_POWER_EVENTS,  // 8
  ...
  NUM_POWER_AUX_SEQUENCE_EVENTS  // 13
} PowerAuxSequenceEvent;

typedef enum {
  POWER_OFF_SEQUENCE_EVENT_BEGIN = NUM_POWER_AUX_SEQUENCE_EVENTS,  // 13
  ...
  NUM_POWER_OFF_SEQUENCE_EVENTS  // 19
} PowerOffSequenceEvent;

typedef enum {
  POWER_MAIN_SEQUENCE_EVENT_BEGIN = NUM_POWER_OFF_SEQUENCE_EVENTS,  // 19
  ...
  NUM_POWER_MAIN_SEQUENCE_EVENTS  // 29
} PowerMainSequenceEvent;

typedef enum {
  DRIVE_FSM_INPUT_EVENT_NEUTRAL = NUM_POWER_MAIN_SEQUENCE_EVENTS,  // 29
  ...
  NUM_DRIVE_FSM_INPUT_EVENTS  // 46
} DriveFsmInputEvent;

typedef enum {
  DRIVE_FSM_OUTPUT_EVENT_DRIVE = NUM_DRIVE_FSM_INPUT_EVENTS,  // 46
  DRIVE_FSM_OUTPUT_EVENT_REVERSE,
  DRIVE_FSM_OUTPUT_EVENT_FAULT,
  DRIVE_FSM_OUTPUT_EVENT_PARKING,
  DRIVE_FSM_OUTPUT_EVENT_NEUTRAL,
  NUM_DRIVE_FSM_OUTPUT_EVENTS  // 51
} DriveFsmOutputEvent;

typedef enum {
  PEDAL_MONITOR_RX_TIMED_OUT = NUM_DRIVE_FSM_OUTPUT_EVENTS,  // 51
  PEDAL_MONITOR_STATE_CHANGE,
  NUM_PEDAL_MONITOR_EVENTS  // 53
} PedalMonitorEvent;

typedef enum {
  BATTERY_HEARTBEAT_EVENT_HEALTH_CHECK_REQUEST = NUM_PEDAL_MONITOR_EVENTS,  // 53
  NUM_BATTERY_HEARTBEAT_EVENTS                                              // 54
} BatteryHeartbeatEvent;

typedef enum {
  CENTRE_CONSOLE_BUTTON_PRESS_EVENT_DRIVE = NUM_BATTERY_HEARTBEAT_EVENTS,  // 54
  ...
  NUM_CENTRE_CONSOLE_BUTTON_PRESS_EVENTS  // 60
} CentreConsoleButtonPressEvent;

typedef enum {
  HAZARD_EVENT_ON = NUM_CENTRE_CONSOLE_BUTTON_PRESS_EVENTS,  // 60
  HAZARD_EVENT_OFF,
  NUM_HAZARD_EVENTS  // 62
} HazardEvent;

typedef enum {
  RACE_SWITCH_EVENT_OFF = NUM_HAZARD_EVENTS,  // 62
  RACE_SWITCH_EVENT_ON,
  NUM_RACE_SWITCH_EVENTS  // 64
} RaceSwitchEvent;

#define NUM_CENTRE_CONSOLE_EVENTS NUM_RACE_SWITCH_EVENTS  // 64
