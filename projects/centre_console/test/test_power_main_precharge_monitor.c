...
void test_precharge_monitor_init_will_raise_event_if_precharge_completes(void) {
  TEST_ASSERT_OK(power_main_precharge_monitor_init(&s_precharge_monitor));
  ...
  CanMessage msg = { .type = CAN_MSG_TYPE_ACK,
                     .msg_id = SYSTEM_CAN_MESSAGE_PRECHARGE_COMPLETE,
                     .source_id = SYSTEM_CAN_DEVICE_MOTOR_CONTROLLER };

  TEST_ASSERT_OK(s_can_ack_status);
}

void test_precharge_monitor_times_out_after_5_seconds_and_raises_event(void) {
  TEST_ASSERT_OK(power_main_precharge_monitor_init(&s_precharge_monitor));
  TEST_ASSERT_OK(power_main_precharge_monitor_start(&s_precharge_monitor));
