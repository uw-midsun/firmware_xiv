...
static void callback(const GpioAddress *address, void *context) {
  // Is it right to pass the GpioAddress A6 by *context? I'm unsure what *address is supposed to
  // return
...
int main(void) {
  ...
  GpioSettings B2_settings = { GPIO_DIR_IN, GPIO_STATE_LOW, GPIO_RES_NONE, GPIO_ALTFN_ANALOG };

  GpioSettings A6_settings = { GPIO_DIR_IN, GPIO_STATE_LOW, GPIO_RES_NONE, GPIO_ALTFN_ANALOG };
