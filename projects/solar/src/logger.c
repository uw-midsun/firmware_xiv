...
static char *s_data_point_types_to_key_format[] = {
  ...
  [DATA_POINT_TYPE_TEMPERATURE] = "MPPT %d temperature",     //
  [DATA_POINT_TYPE_MPPT_PWM] = "MPPT %d PWM",                //
  [DATA_POINT_TYPE_CR_BIT] = "MPPT %d CR bit",               //
};

static char *s_data_point_types_to_value_format[] = {
  [DATA_POINT_TYPE_MPPT_VOLTAGE] = "%d mV",  //
  [DATA_POINT_TYPE_VOLTAGE] = "%d mV",       //
  [DATA_POINT_TYPE_MPPT_CURRENT] = "%d mV",  //
  [DATA_POINT_TYPE_CURRENT] = "%d uA",       //
  [DATA_POINT_TYPE_TEMPERATURE] = "%d dC",   //
  [DATA_POINT_TYPE_MPPT_PWM] = "%d/1000",    //
  [DATA_POINT_TYPE_CR_BIT] = "%d",           //
};
...
static void prv_log_data_point(DataPointType type, Mppt mppt, DataPoint data_point) {
  ...
  if (is_set) {
    uint32_t value;
    data_store_get(data_point, &value);
    snprintf(value, SIZEOF_ARRAY(value), s_data_point_types_to_value_format[type], value);
  }
