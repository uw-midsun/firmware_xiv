...
AdcChannel aux_channels[3] = { ADC_CHANNEL_0, ADC_CHANNEL_1, ADC_CHANNEL_2 };

static uint16_t s_status = 0;
static uint16_t s_aux_volt = 0;
static uint16_t s_aux_temp = 0;
uint16_t prv_checker() {
  adc_read_raw(aux_channels[0], &s_aux_volt);
  adc_read_raw(aux_channels[1], &s_aux_temp);
...
StatusCode aux_dcdc_monitor_init() {
  for (int i = 0; i < 2; ++i) {  // only 2 since we don't use that last one
