...
static GpioAddress s_dcdc_address = {
...
AdcChannel aux_channels[3] = { NUM_ADC_CHANNELS, NUM_ADC_CHANNELS,
                               NUM_ADC_CHANNELS };
// again not used currently
AdcChannel dcdc_channels[3] = { NUM_ADC_CHANNELS, NUM_ADC_CHANNELS,
                                NUM_ADC_CHANNELS };  
...
static void prv_power_selection_callback(SoftTimerId timer_id, void *context) {
  ...
  CAN_TRANSMIT_AUX_BATTERY_STATUS(aux_volt, aux_temp, status);
