...
static StatusCode prv_test_adc_read_callback_handler(const CanMessage *msg, void *context) {
  TEST_ASSERT_EQUAL(SYSTEM_CAN_MESSAGE_BABYDRIVER, msg->msg_id);
  s_received_context = context;

  // todo RAIYAN
  // one for raw read, another for converted read
  if (s_times_callback_called < 2) {
    CAN_UNPACK_BABYDRIVER(msg, NULL, s_received_data[0], s_received_data[1], s_received_data[2],
