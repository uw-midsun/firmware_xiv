...
StatusCode config_init(void) {
  status_ok_or_return(persist_init(&s_config_1_persist, BOOTLOADER_CONFIG_PAGE_1_FLASH_PAGE,
                                   &s_config_1_blob, sizeof(s_config_1_blob), false));
  ...
  status_ok_or_return(persist_init(&s_config_2_persist, BOOTLOADER_CONFIG_PAGE_2_FLASH_PAGE,
                                   &s_config_2_blob, sizeof(s_config_2_blob), false));
  ...
  uint32_t config_1_check_crc = crc32_arr((uint8_t *)&s_config_1_blob, sizeof(s_config_1_blob));
  uint32_t config_2_check_crc = crc32_arr((uint8_t *)&s_config_2_blob, sizeof(s_config_2_blob));
  ...
  if (is_config_1_equal && is_config_2_equal) {
    ...
    s_config_1_blob.CRC32 = config_1_crc;
    memcpy(&s_config_2_blob, &s_config_1_blob, sizeof(s_config_1_blob));
    persist_commit(&s_config_2_persist);
  } else {
    s_config_2_blob.CRC32 = config_2_crc;
    memcpy(&s_config_1_blob, &s_config_2_blob, sizeof(s_config_2_blob));
...
StatusCode config_get(BootloaderConfig *config) {
  ...
  memcpy(config, &s_config_1_blob, sizeof(s_config_1_blob));
...
StatusCode config_commit(BootloaderConfig *input_config) {
  ...
  uint32_t input_config_crc = crc32_arr((uint8_t *)input_config, sizeof(input_config));
  input_config->CRC32 = input_config_crc;

  memcpy(&s_config_1_blob, input_config, sizeof(*input_config));
  persist_commit(&s_config_1_persist);

  status_ok_or_return(persist_init(&s_config_1_persist, BOOTLOADER_CONFIG_PAGE_1_FLASH_PAGE,
                                   &s_config_1_blob, sizeof(s_config_1_blob), false));

  bool input_equals_config_1 = memcmp(&s_config_1_blob, input_config, sizeof(*input_config)) == 0;
