...
static StatusCode prv_callback_update_name(uint8_t *data, uint16_t data_len, void *context) {
  ...
  status = pb_decode(&stream, UpdateName_fields, &name_proto)
...
StatusCode update_name_init() {
  status_ok_or_return(dispatcher_register_callback(BOOTLOADER_DATAGRAM_UPDATE_NAME,
                                                   prv_callback_update_name, NULL));
}
