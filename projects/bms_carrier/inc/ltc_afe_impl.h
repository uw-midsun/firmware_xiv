  ...
  //LOG_DEBUG("%d\n", k);
    ...
    for (uint8_t device = 0; device < PLUTUS_CFG_AFE_DEVICES_IN_CHAIN; ++device) {
      ...
      for (uint16_t cell = 0; cell < LTC6804_CELLS_IN_REG; ++cell) {
        ...
        int volt=voltage;
        LOG_DEBUG("Voltage: %d\n", volt);
        LOG_DEBUG("device cell:%d\n", device_cell);
        LOG_DEBUG("Index: %d\n", index);
        if (((afe->cell_bitset[device] >> device_cell) & 0x1) == 0x1) {
          LOG_DEBUG("do it be storin doe?\n");
      ...
      //LOG_DEBUG("values of received before change: %d\n", voltage_register[device].pec);
      //LOG_DEBUG("values of crc before change: %d\n", voltage_register[device]);
      ...
      if(received_pec != 65535) {
        ret_voltage_ok = true;
        LOG_DEBUG("values of received %d\n", received_pec);
        LOG_DEBUG("values of crc? %d\n", data_pec);
      }
      if(ret_voltage_ok) {
        for(int i = 0; i < 10; i++) 
        LOG_DEBUG("DIDNT RETURN 65535\n");
      }
      if (received_pec != data_pec) {
        LOG_DEBUG("do it be failin doe?\n");
...
StatusCode ltc_afe_impl_read_aux(LtcAfeStorage *afe, uint8_t device_cell) {
  LOG_DEBUG("ltc_afe_impl_read_aux\n");
