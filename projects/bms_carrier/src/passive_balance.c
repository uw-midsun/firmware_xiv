...
StatusCode passive_balance_init(LtcAfeStorage *storage) {
  return soft_timer_start_millis(PASSIVE_BALANCE_INTERVAL_MS, passive_balance, storage, NULL);
}

void passive_balance(SoftTimerId timer_id, void *context) {
  // Storage for max + min voltage values as well as the cell # with the max voltage.  All default
  // to the first value in the array. Should probably put these in a struct
