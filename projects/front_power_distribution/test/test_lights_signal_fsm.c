...
// Make sure left->right and right->left is possible.
// Test transitioning: none -> left -> right -> none -> right -> left -> none
// Events given: left on, right on, right off, right on, left on, left off
void test_lights_signal_fsm_left_to_right(void) {
  TEST_ASSERT_OK(lights_signal_fsm_init(&s_storage, &s_settings));
...
// Make sure hazard-left->hazard-right and hazard-right->hazard-left is possible.
// Test transitioning: none -> left -> hazard-left -> hazard-right -> hazard-left -> left -> none
// Events given: left on, hazard on, right on, left on, hazard off, left off
void test_lights_signal_fsm_hazardleft_hazardright(void) {
  TEST_ASSERT_OK(lights_signal_fsm_init(&s_storage, &s_settings));
  ...
  // right on: transition to hazard-right (no behaviour change)
  TEST_ASSERT_OK(lights_signal_fsm_process_event(&s_storage, &s_right_on_event));
  TEST_LIGHTS_SIGNAL_ASSERT_NO_EVENT_RAISED();
